from tkinter import (Button, Label, Frame, Tk, RAISED, NSEW,
                     Entry, ttk, RIGHT, E)
from functions import make_temp_file, search_freq_id_edit, only_number
...
class Window(Frame):  # All the stuff for the GUI
    ...
    def init_window(self):
        ...
        def make_freq_temp():
            ...
            if search_freq_id_edit(file_name, hc_e.get()) is False:
                line = False
            else:
                line, order = search_freq_id_edit(file_name, hc_e.get())
                line['x'] = order
                line['file_name'] = file_name
            # if found print in freq
            if line:
                make_temp_file('freq_edit', line)
                print(file_name)
                path = os.path.join(__location__, 'freq_edit.py')
        ...
        encryptModeButton = Button(button_frame, command=make_freq_temp,
                                   text="تفاصيل التعديل", width=20, height=2)
